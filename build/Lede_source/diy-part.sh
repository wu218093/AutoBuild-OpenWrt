...
sed -i "/uci commit network/i\uci set network.lan.ipaddr='192.168.50.252'" $ZZZ                      # IPv4 地址(openwrt后台地址)
...
sed -i "/uci commit network/i\uci set network.lan.dns='192.168.50.252'" $ZZZ                      # DNS(多个DNS要用空格分开)
...
sed -i "/uci commit system/i\uci set system.@system[0].hostname='xmyr-OpenWrt'" $ZZZ            # 修改主机名称为x86-64-OpenWrt
